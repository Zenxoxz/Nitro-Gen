    ...
    for i in range(num):
        try:
            ...
            if response.status_code == 200:
                print(
                    f"{w}[{Fore.GREEN}!{w}] {Fore.GREEN}VALID NITRO{w} : {url}"
                )
                if webhook:
                    try:
                ...
                print(
                    f"{y}[{Fore.LIGHTRED_EX}!{y}]{w} Rate limited, please wait {response.json()['retry_after']/1000} s"
                )
                time.sleep(response.json()['retry_after'] / 1000)
            else:
                print(
                    f"{w}[{Fore.RED}!{w}] {Fore.RED}INVALID NITRO{w} : {url}")
                invalid += 1
        ...
        if os.name == "nt":
            ctypes.windll.kernel32.SetConsoleTitleW(
                f"Nitro Generator and Checker - {len(valid)} Valid | {invalid} Invalid - Made by Zenxoxz"
            )
